...
const StartupIndex = ({ user }) => {
  ...
  useEffect(() => {
    dispatch(getStartupThunk(id)).then(() => {
      if (startup.favorited === true) {
        setIsFavorite(true);
      }
    });
  }, [dispatch, id]);
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.top}>
        ...
        <div className={styles.right}>
          <div className={styles.right_top}>
            <h2>
              {startup?.name}{" "}
              {(user && !user.default_startup && isFavorite === false) ||
              !user ? (
                <i
                  className={`fa-regular fa-heart ${styles.favorite}`}
                  onClick={handleFavorite}
                ></i>
              ) : user && !user.default_startup && isFavorite === true ? (
                <i
                  className={`fa-solid fa-heart ${styles.favorite}`}
      ...
      <div className={styles.bottom}>
        <h3>About</h3>
        <p className={styles.about}>{startup?.description}</p>
      </div>
